...
function custom_artifact_install() {
    ...
    curl --fail-with-body -sSL http://localhost:8081/api/agent/measurements | jq -rc '.[].uuid' | while read uuid; do
        echo "delete measurement uuid = $uuid"
        curl --fail-with-body -sSL -X DELETE http://localhost:8081/api/agent/measurements/$uuid
    done
}
